...
<form class="form-inline definewidth m20" action="role.html" method="get">  
    ...
    <a href="<?php echo U('Admin/Fields/addField');?>" type="button" class="btn btn-success various" data-fancybox-type="iframe">新增字段</a>
</form>
    ...
    <tr>
        ...
        <th width="20%">操作</th>
	     ...
	     <?php if(is_array($fields)): foreach($fields as $key=>$v): ?><tr>
                ...
                <td>
                    <a href="<?php echo U('Admin/Fields/editField',array('fid'=>$v['id']));?>" class="various" data-fancybox-type="iframe">修改</a>
                    &nbsp;&nbsp;&nbsp;
                    <a fieldid="<?php echo ($v['id']); ?>" class="del">删除</a>
                </td>
...
$(document).ready(function(){
   ...
   $(".del").click(function(){
      ...
      var d = dialog({
        ...
        okValue: '确定',
        ok: function () {
                $.post("<?php echo U('Admin/Fields/delField','','');?>",{id:""+fieldid+""},function(status){
                    if(status){
                        var d = dialog({
                                            content: '恭喜，删除成功咯;-)'
                                        });
